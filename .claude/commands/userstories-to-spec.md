...
**You MUST write physical files to disk. This is not optional.**

Every run of this skill MUST produce these files before finishing:

```
docs/user-stories/{US-ID}.md      ← one file per user story — REQUIRED
docs/{feature}-api-spec-{date}.md  ← combined spec — REQUIRED
```

**Never stop after generating content in the chat.**
**Never treat showing content in chat as a substitute for writing files.**
**Step 6 is not optional — it MUST run after Step 5 every single time.**

The execution order is strictly:
```
Step 1 → Step 2 → Step 3 → Step 4 → Step 5 → Step 6 (WRITE FILES) → Step 7
```
Step 6 writes files. Step 7 prints the summary.
**The skill is not complete until Step 6 has run and all files exist on disk.**
...
## Step 5 — Generate API Spec Content (in memory)

**This step prepares content only. Step 6 writes it to disk.**
**Do NOT stop here — proceed directly to Step 6 after generating content.**

Generate the full API spec content using this structure:
...
**→ Content generated. Now IMMEDIATELY proceed to Step 6 to write files to disk.**
**Do not show content in chat and stop. Write the files.**

---

## Step 6 — Write Output Files ← THIS STEP IS MANDATORY

**This step MUST run. Writing files is the purpose of this skill.**

Write output in two places:

### 6.1 Create docs folder structure

Use the `Write` tool to create the folder structure by writing a placeholder file first, then the real files:
...
This ensures both folders exist before writing user story files.

**Alternatively use bash if available:**
```bash
...
**The folders MUST exist before Step 6.2 runs.**

### 6.2 Write one .md file per user story into docs/user-stories/

**One `Write` tool call per user story — no exceptions.**
**Use the REAL content generated in Step 5 — not placeholder text.**
**Every `{placeholder}` must be replaced with actual values before writing.**

For each user story found (e.g. US-23, US-24, US-25), execute:

```
Write("docs/user-stories/{US-ID}.md", <full file content>)
```

Where `<full file content>` is the complete populated markdown — no placeholders left:

```markdown
# {US-ID} — {Short Title}

**Actor:** {actor extracted from story}
**Source:** {requirements filename}
**Date:** {today's date}
**Status:** Open
...
|---|---|---|---|
| {HTTP method} | {full path} | {what it does} | `{Permission.Constant}` |

*(List every endpoint from Step 4 that belongs to this user story)*
...
**Permission:** `{Permission.Constant}` — {who has it}

**Request Body:**

| Field | Type | Required | Description | Constraints |
|---|---|---|---|---|
| {field1} | {type} | Yes/No | {description} | {constraints} |
...
|---|---|---|
| `{Resource}.{Action}` | {roles} | {endpoint} |
...
| Rule | Enforced At |
|---|---|
| {BR extracted from story} | {endpoint} |

---

## Linked Tasks

*(Populated by api-spec-to-technical-plan skill)*

| Task | Title | Layer | Estimate |
|---|---|---|---|
| — | Not yet generated | — | — |
...
**Repeat the `Write` call for EVERY user story found — one file per story.**

### 6.3 Write the combined API Spec file

```
Write(
  "docs/{feature-name}-api-spec-{YYYY-MM-DD}.md",
  <full content from Step 5 — all endpoints, permissions, validation rules,
   business rules, open questions — fully populated, no placeholders>
)
```

This is the file that feeds into `api-spec-to-technical-plan`.

### 6.4 Verify all files were written

After all `Write` calls, verify every file exists:
...
**If any file is missing — write it again before proceeding to Step 7.**
**Do not print the summary until all files are confirmed on disk.**
...
Output Files:
  docs/user-stories/
    ✓ US-001.md   — User can add address
    ✓ US-002.md   — User can update address
    ✓ US-003.md   — Address validated before saving

  docs/
    ✓ customer-management-api-spec-2025-03-16.md  (combined)
